@if (Laravel\Fortify\Features::enabled(Laravel\Fortify\Features::twoFactorAuthentication()))
    <div class="mt-8">
        ...
        @if (auth()->user()->two_factor_secret)
            <form method="POST" action="{{ url('user/two-factor-authentication') }}" style="display:inline;">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded">Disable 2FA</button>
            </form>
            <form method="POST" action="{{ url('user/two-factor-authentication') }}" style="display:inline; margin-left: 10px;">
                @csrf
