      ...
      <div>
          ...
          <nav class="w-full flex gap-4 flex-wrap justify-center mt-4 mb-8">
              ...
              <a href="{{ route('seller.products') }}" class="inline-block px-8 py-3 text-lg font-semibold text-black dark:text-white rounded-md leading-normal transition hover:text-red-500 hover:drop-shadow-[0_0_8px_rgba(239,68,68,0.7)] focus:outline-none">Products</a>
                <a href="{{ route('orders.notifications') }}" class="inline-block px-8 py-3 text-lg font-semibold text-black dark:text-white rounded-md leading-normal transition hover:text-red-500 hover:drop-shadow-[0_0_8px_rgba(239,68,68,0.7)] focus:outline-none"> orders </a>
          </nav>
                          ...
                          @if($product->category === $cat)
                              <div class="bg-white dark:bg-gray-700 rounded-lg shadow-md p-4 flex flex-col items-center product-card cursor-pointer relative group min-w-0 w-full max-w-xs mx-auto transition-transform duration-200 hover:scale-105"
                               ...
                               data-product-image="{{ $product->image ? asset('products/' . $product->image) : '' }}"
                                  ...
                                  @if($product->image)
                                      <img src="{{ asset('products/' . $product->image) }}" alt="{{ $product->name }}" class="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 object-cover object-center rounded mb-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700" />
                                  @else
